...
def test_copy_duplicates_lesson_and_experiments_leaving_original(as_user, admin):
    ...
    resp = as_user(admin).post(
        f"/api/lessons/{source.id}/move-copy/", {"mode": "copy", "grade": "5-6", "chorak": 1}, format="json"
    )
    ...
    assert copy.hafta == 2  # appended after the existing grade-5-6 lesson
...
def test_move_relocates_and_renumbers_both_grades(as_user, admin):
    ...
    resp = as_user(admin).post(
        f"/api/lessons/{source.id}/move-copy/", {"mode": "move", "grade": "5-6", "chorak": 2}, format="json"
    )
    assert resp.status_code == 200
    ...
    # grade 5-6 now has both lessons, appended after the existing one
    grade56 = list(Lesson.objects.filter(grade="5-6").order_by("hafta"))
    assert [l.hafta for l in grade56] == [1, 2]
    assert grade56[1].id == source.id
