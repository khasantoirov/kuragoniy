...
def test_creating_a_lesson_notifies_dev(mock_notify, as_user, admin):
    ...
    resp = client.post(
        '/api/lessons/',
        {'title': 'Test dars', 'grade': '7-8-9', 'chorak': 1, 'hafta': 1, 'goal': ''},
        format='json',
    )
...
def test_deleting_a_lesson_notifies_dev(mock_notify, as_user, admin):
    lesson = Lesson.objects.create(title="O'chiriladigan", grade='7-8-9', chorak=1, hafta=1)
    client = as_user(admin)
...
def test_editing_a_lesson_shows_before_and_after_per_field(mock_notify, as_user, admin):
    lesson = Lesson.objects.create(title='Eski nom', grade='7-8-9', chorak=1, hafta=1, goal='Eski maqsad')
    client = as_user(admin)
...
def test_editing_a_lesson_without_real_changes_does_not_notify(mock_notify, as_user, admin):
    lesson = Lesson.objects.create(title='O\'zgarmas', grade='7-8-9', chorak=1, hafta=1)
    client = as_user(admin)
...
def test_deleting_a_lesson_lists_its_experiments_and_goal(mock_notify, as_user, admin):
    lesson = Lesson.objects.create(title="O'chiriladigan", grade='7-8-9', chorak=1, hafta=1, goal='Muhim maqsad')
...
def test_moving_a_lesson_shows_original_and_new_location(mock_notify, as_user, admin):
    lesson = Lesson.objects.create(title='Ko\'chadigan', grade='7-8-9', chorak=1, hafta=1)
    client = as_user(admin)
    resp = client.post(f'/api/lessons/{lesson.id}/move-copy/', {'mode': 'move', 'grade': '5-6', 'chorak': 3}, format='json')
    assert resp.status_code == 200
    mock_notify.assert_called_once()
    text = mock_notify.call_args[0][1]
    assert '7-8-9-sinf, 1-chorak' in text
    assert '5-6-sinf, 3-chorak' in text
...
def test_teacher_action_never_notifies_dev(mock_notify, as_user, teacher):
    ...
    resp = client.post(
        '/api/lessons/',
        {'title': 'Ruxsatsiz', 'grade': '7-8-9', 'chorak': 1, 'hafta': 1, 'goal': ''},
        format='json',
    )
